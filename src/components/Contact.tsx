...
export default function Contact({ locale, standalone = false }: { locale: Locale; standalone?: boolean }) {
  ...
  return (
    <section
      id="contact"
      className={`bg-[var(--color-panel)] border-t border-[var(--color-line)] ${standalone ? "grid-bg pt-[136px] pb-24" : "py-20 md:py-28"}`}
    >
      <div className="container grid lg:grid-cols-[1fr_1.05fr] gap-12 lg:gap-16 items-start">
        <Reveal>
          ...
          {standalone ? (
            <h1 className="text-[2.6rem] leading-[1.06] sm:text-5xl font-semibold tracking-[-0.035em]">{t.title}</h1>
          ) : (
            <h2 className="text-3xl md:text-[2.6rem] leading-[1.1] font-semibold tracking-[-0.03em]">{t.title}</h2>
          )}
          <p className="mt-6 max-w-[46ch] text-lg leading-relaxed text-[var(--color-slate)]">{t.lead}</p>

          <dl className="mt-10 space-y-5 border-t border-[var(--color-line)] pt-8">
            <div>
        ...
        <Reveal delay={120}>
          <div className="rounded-[var(--radius-xl)] border border-[var(--color-line)] bg-white p-7 shadow-[var(--shadow-card)] md:p-9">
